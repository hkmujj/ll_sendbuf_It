...
<%
	...
	//获取公共参数
	String taskid = "1705454021";
	String phone = "13316083349";
	...
	rcobj.put("Phone", phone);
	rcobj.put("SetCode", packagecode);
	rcobj.put("AreaCode", "0000");
	...
	String result = "";
	byte[] aesKey = sha256(versionC);

	logger.info("aesKey.len = " + aesKey.length);
	...
	String K = base64(encrypt(
			loadPrivateKey(privateKey), aesKey));
	K = java.net.URLEncoder.encode(K, "utf-8");
	byte[] data = aseEncrypt(Cno, aesKey, iv);

	logger.info("data.len = " + data.length);

	String C = base64(data);
	...
	try {
		...
		outpr = new PrintWriter(conn.getOutputStream());
		// 发送请求参数
	...
	try {
		String resultderet = null;

		logger.info("result.len = " + result.length());

		byte[] baseResult = basedecode(result);

		byte[] aesResult = aesdecrypt(baseResult, aesKey, iv);

		resultderet = new String(aesResult, "UTF-8");
		ret = resultderet;
	...
	if (ret != null && ret.trim().length() > 0) {
		...
		errmap.put("0108", "套餐不可用 合作伙伴与骏伯公司签订的商务合同中不包含该套餐");
