...
%><%
	...
	while(true){
		...
		json.put("package", packagecode);
		json.put("sign", MD5Util.getUpperMD5(account + api_key));
		
		logger.info("weike send json = " + json.toString());
		...
		if (ret != null && ret.trim().length() > 0) {
			logger.info("weike send ret = " + ret);
			try {
				JSONObject retjson = JSONObject.fromObject(ret);
				String code = retjson.getString("result_code");
				String message = retjson.getString("result_msg");
				if(code.equals("0")){
					request.setAttribute("result", "success");
					request.setAttribute("reportid", retjson.getString("msg_id"));
				}else{
