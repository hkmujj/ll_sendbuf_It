...
<%
	out.clearBuffer();
	//报警线
	double YY_YX_successrate = 0.85;//行业+营销
	double YY_successrate = 0.90;//行业
	double YX_successrate = 0.80;//营销
	...
	try{
		...
		try{
			...
			String sql = "select a.routeid,sum(a.totalsended) as totalsended,sum(a.totalsuccess) as totalsuccess,sum(a.totalfail) as totalfail,b.name from sms_dayreport as a left join sms_routes as b on a.routeid=b.routeid where a.rdate="+getDateString()+" and a.totalsended>500 group by a.routeid";
			...
			while(rs.next()){
				...
				if(name.indexOf("行业+营销") >= 0){
					if(routesu_rate < YY_YX_successrate){
					...
					if(routesu_rate < YX_successrate){
						node.put("routeid", rs.getString("routeid"));
						node.put("name", "营销");
						node.put("totalsended", totalsended);
						node.put("totalsuccess_rate", nFromat.format(routesu_rate));
			...
			for(int y = 0; y < rateary.size(); y++){
				...
				alarmreason.add("[" + routeid + "]" + name + "成功率过低，发送数" + totalsended + "，成功率" + totalsuccess_rate + "，失败率" + totalfail_rate);
			}
			rs.close();
			rs = null;
			psm.close();
