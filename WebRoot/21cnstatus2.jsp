		...
		for(int i = 0; i < idarray.length; i++){
	        ...
	        try {
				...
				if(pagestr.indexOf("流量兑换处理中") > -1){
		        	...
		        	System.out.println("uuid = " + idarray[i] + ", info = " + "流量兑换失败");
		        	JSONObject rp = new JSONObject();
					rp.put("code", 1);
					rp.put("message", "失败");
					rp.put("resp", "");
					obj.put(idarray[i], rp);
					logger.info("uuid = " + idarray[i] + ", check page = " + pagestr);
		        }else{
		        	String ddtime = idarray[i].substring(0, 14);
					SimpleDateFormat df = new SimpleDateFormat("yyyyMMddHHmmss");
					long l = System.currentTimeMillis() - df.parse(ddtime).getTime();
					if(l > 600000){
				        HtmlElement ele = _21cnpage.getHtmlElementById("j-load");
		        		String style = null;
		        		try{
		        			style = ele.getAttribute("style"); 
		        		} catch (Exception e) {
		        		}
		        		if(style != null && style.indexOf("display: block") > -1){
		        			JSONObject rp = new JSONObject();
							rp.put("code", 1);
							rp.put("message", "失败");
							rp.put("resp", "");
							...
							logger.info("uuid = " + idarray[i] + ", check style = " + style);
		        		}
	        		}
        		}
			} catch (Exception e) {
