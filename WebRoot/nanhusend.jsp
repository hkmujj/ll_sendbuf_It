...
%><%
	...
	while(true){
		...
		String signbef = "account=" + account + "&mobile=" + phone + "&package=" + packagecode;
		String sign = MD5Util.getLowerMD5(signbef + "&key=" + key);
		 url = url + "v=1.1&action=charge&" + signbef + "&sign=" + sign;
		//在执行请求前先获取连接, 防止访问通道线程超量
		Cache.getConnection(routeid);
		try {
		 ret = HttpAccess.postNameValuePairRequest(url, new HashMap<String, String>(), "utf-8", "liuliangsky");
		} catch (Exception e) {
			e.printStackTrace();
